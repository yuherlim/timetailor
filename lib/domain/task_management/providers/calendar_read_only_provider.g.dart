// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'calendar_read_only_provider.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$smallScreenTimeSlotHeightHash() =>
    r'226718fe667122ebe009aee4172783ee6de0175e';

/// See also [smallScreenTimeSlotHeight].
@ProviderFor(smallScreenTimeSlotHeight)
final smallScreenTimeSlotHeightProvider = AutoDisposeProvider<double>.internal(
  smallScreenTimeSlotHeight,
  name: r'smallScreenTimeSlotHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$smallScreenTimeSlotHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SmallScreenTimeSlotHeightRef = AutoDisposeProviderRef<double>;
String _$bigScreenTimeSlotHeightHash() =>
    r'3eeaed59a6293927b408c84e63a20e240cdd88fb';

/// See also [bigScreenTimeSlotHeight].
@ProviderFor(bigScreenTimeSlotHeight)
final bigScreenTimeSlotHeightProvider = AutoDisposeProvider<double>.internal(
  bigScreenTimeSlotHeight,
  name: r'bigScreenTimeSlotHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$bigScreenTimeSlotHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef BigScreenTimeSlotHeightRef = AutoDisposeProviderRef<double>;
String _$defaultTimeSlotHeightHash() =>
    r'6135b9c15c83f6c67c95b654670b9999f0b93cc8';

/// See also [defaultTimeSlotHeight].
@ProviderFor(defaultTimeSlotHeight)
final defaultTimeSlotHeightProvider = AutoDisposeProvider<double>.internal(
  defaultTimeSlotHeight,
  name: r'defaultTimeSlotHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$defaultTimeSlotHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DefaultTimeSlotHeightRef = AutoDisposeProviderRef<double>;
String _$snapIntervalMinutesHash() =>
    r'666676eb61c002096e6444b3e68bbe2510f5e4c1';

/// See also [snapIntervalMinutes].
@ProviderFor(snapIntervalMinutes)
final snapIntervalMinutesProvider = AutoDisposeProvider<double>.internal(
  snapIntervalMinutes,
  name: r'snapIntervalMinutesProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$snapIntervalMinutesHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SnapIntervalMinutesRef = AutoDisposeProviderRef<double>;
String _$snapIntervalHeightHash() =>
    r'80fcd8d18d84f5b4205cf0f6a4aeea0c252326aa';

/// See also [snapIntervalHeight].
@ProviderFor(snapIntervalHeight)
final snapIntervalHeightProvider = AutoDisposeProvider<double>.internal(
  snapIntervalHeight,
  name: r'snapIntervalHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$snapIntervalHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SnapIntervalHeightRef = AutoDisposeProviderRef<double>;
String _$calendarHeightHash() => r'6bd18e9ca01bd532595b0f5ee4d63c5cc0dd7052';

/// See also [calendarHeight].
@ProviderFor(calendarHeight)
final calendarHeightProvider = AutoDisposeProvider<double>.internal(
  calendarHeight,
  name: r'calendarHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$calendarHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CalendarHeightRef = AutoDisposeProviderRef<double>;
String _$calendarWidgetTopBoundaryYHash() =>
    r'0999b6033bafa255643647bdeda40eecd7b028b3';

/// See also [calendarWidgetTopBoundaryY].
@ProviderFor(calendarWidgetTopBoundaryY)
final calendarWidgetTopBoundaryYProvider = AutoDisposeProvider<double>.internal(
  calendarWidgetTopBoundaryY,
  name: r'calendarWidgetTopBoundaryYProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$calendarWidgetTopBoundaryYHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CalendarWidgetTopBoundaryYRef = AutoDisposeProviderRef<double>;
String _$calendarBottomPaddingHash() =>
    r'4356f6e7a88780a81d695f5a066a246028ccba77';

/// See also [calendarBottomPadding].
@ProviderFor(calendarBottomPadding)
final calendarBottomPaddingProvider = AutoDisposeProvider<double>.internal(
  calendarBottomPadding,
  name: r'calendarBottomPaddingProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$calendarBottomPaddingHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CalendarBottomPaddingRef = AutoDisposeProviderRef<double>;
String _$calendarWidgetBottomBoundaryYHash() =>
    r'013fb58e1de55157b004af4cd7815595b859f355';

/// See also [calendarWidgetBottomBoundaryY].
@ProviderFor(calendarWidgetBottomBoundaryY)
final calendarWidgetBottomBoundaryYProvider =
    AutoDisposeProvider<double>.internal(
  calendarWidgetBottomBoundaryY,
  name: r'calendarWidgetBottomBoundaryYProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$calendarWidgetBottomBoundaryYHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CalendarWidgetBottomBoundaryYRef = AutoDisposeProviderRef<double>;
String _$timeSlotBoundariesHash() =>
    r'7d57e521edbbda4af5848f06213c5b7653e7eeeb';

/// See also [timeSlotBoundaries].
@ProviderFor(timeSlotBoundaries)
final timeSlotBoundariesProvider = AutoDisposeProvider<List<double>>.internal(
  timeSlotBoundaries,
  name: r'timeSlotBoundariesProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$timeSlotBoundariesHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef TimeSlotBoundariesRef = AutoDisposeProviderRef<List<double>>;
String _$timePeriodsHash() => r'6322c2fd069537b4b1d44ffbae1358fa95fe238f';

/// See also [timePeriods].
@ProviderFor(timePeriods)
final timePeriodsProvider = AutoDisposeProvider<List<String>>.internal(
  timePeriods,
  name: r'timePeriodsProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$timePeriodsHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef TimePeriodsRef = AutoDisposeProviderRef<List<String>>;
String _$timeIndicatorIconSizeHash() =>
    r'ef2e9bde7f2f755efe68474b119552ed788064f9';

/// See also [timeIndicatorIconSize].
@ProviderFor(timeIndicatorIconSize)
final timeIndicatorIconSizeProvider = AutoDisposeProvider<double>.internal(
  timeIndicatorIconSize,
  name: r'timeIndicatorIconSizeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$timeIndicatorIconSizeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef TimeIndicatorIconSizeRef = AutoDisposeProviderRef<double>;
String _$draggableBoxIndicatorWidthHash() =>
    r'27a2dfdded63fc3933d9ce08c5ca1396d2553ec6';

/// See also [draggableBoxIndicatorWidth].
@ProviderFor(draggableBoxIndicatorWidth)
final draggableBoxIndicatorWidthProvider = AutoDisposeProvider<double>.internal(
  draggableBoxIndicatorWidth,
  name: r'draggableBoxIndicatorWidthProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$draggableBoxIndicatorWidthHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DraggableBoxIndicatorWidthRef = AutoDisposeProviderRef<double>;
String _$draggableBoxIndicatorHeightHash() =>
    r'6cb6d8914be8379be5ff0102bf55e7b8ff5fe29a';

/// See also [draggableBoxIndicatorHeight].
@ProviderFor(draggableBoxIndicatorHeight)
final draggableBoxIndicatorHeightProvider =
    AutoDisposeProvider<double>.internal(
  draggableBoxIndicatorHeight,
  name: r'draggableBoxIndicatorHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$draggableBoxIndicatorHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DraggableBoxIndicatorHeightRef = AutoDisposeProviderRef<double>;
String _$draggableBoxIndicatorIconSizeHash() =>
    r'207c2738a69304dc60d80f05478a457580f1c561';

/// See also [draggableBoxIndicatorIconSize].
@ProviderFor(draggableBoxIndicatorIconSize)
final draggableBoxIndicatorIconSizeProvider =
    AutoDisposeProvider<double>.internal(
  draggableBoxIndicatorIconSize,
  name: r'draggableBoxIndicatorIconSizeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$draggableBoxIndicatorIconSizeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DraggableBoxIndicatorIconSizeRef = AutoDisposeProviderRef<double>;
String _$defaultDragIndicatorWidthHash() =>
    r'f67a5214a97e0d55d1fb31112f527115c937a18c';

/// See also [defaultDragIndicatorWidth].
@ProviderFor(defaultDragIndicatorWidth)
final defaultDragIndicatorWidthProvider = AutoDisposeProvider<double>.internal(
  defaultDragIndicatorWidth,
  name: r'defaultDragIndicatorWidthProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$defaultDragIndicatorWidthHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DefaultDragIndicatorWidthRef = AutoDisposeProviderRef<double>;
String _$defaultDragIndicatorHeightHash() =>
    r'e80abc2d96fa679d4a56a468ffb1f98e6b51d5a9';

/// See also [defaultDragIndicatorHeight].
@ProviderFor(defaultDragIndicatorHeight)
final defaultDragIndicatorHeightProvider = AutoDisposeProvider<double>.internal(
  defaultDragIndicatorHeight,
  name: r'defaultDragIndicatorHeightProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$defaultDragIndicatorHeightHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DefaultDragIndicatorHeightRef = AutoDisposeProviderRef<double>;
String _$dragIndicatorIconSizeHash() =>
    r'15a09393e06754abed4ad78f01d9d9652f3842ab';

/// See also [dragIndicatorIconSize].
@ProviderFor(dragIndicatorIconSize)
final dragIndicatorIconSizeProvider = AutoDisposeProvider<double>.internal(
  dragIndicatorIconSize,
  name: r'dragIndicatorIconSizeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$dragIndicatorIconSizeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DragIndicatorIconSizeRef = AutoDisposeProviderRef<double>;
String _$autoScrollAmountHash() => r'8fc64ce4095e37dbfd55773414d0d488b168374d';

/// See also [autoScrollAmount].
@ProviderFor(autoScrollAmount)
final autoScrollAmountProvider = AutoDisposeProvider<double>.internal(
  autoScrollAmount,
  name: r'autoScrollAmountProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$autoScrollAmountHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef AutoScrollAmountRef = AutoDisposeProviderRef<double>;
String _$initialBottomSheetExtentHash() =>
    r'11f6f5148d4b09a4ff9c3deffeda0ef642a9fce4';

/// See also [initialBottomSheetExtent].
@ProviderFor(initialBottomSheetExtent)
final initialBottomSheetExtentProvider = AutoDisposeProvider<double>.internal(
  initialBottomSheetExtent,
  name: r'initialBottomSheetExtentProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$initialBottomSheetExtentHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef InitialBottomSheetExtentRef = AutoDisposeProviderRef<double>;
String _$minBottomSheetExtentHash() =>
    r'f26698cd2883d623612b0081291c4af759698576';

/// See also [minBottomSheetExtent].
@ProviderFor(minBottomSheetExtent)
final minBottomSheetExtentProvider = AutoDisposeProvider<double>.internal(
  minBottomSheetExtent,
  name: r'minBottomSheetExtentProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$minBottomSheetExtentHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef MinBottomSheetExtentRef = AutoDisposeProviderRef<double>;
String _$middleBottomSheetExtentHash() =>
    r'607cef96342244f52e47839dd6bd7c98c1cc1f0a';

/// See also [middleBottomSheetExtent].
@ProviderFor(middleBottomSheetExtent)
final middleBottomSheetExtentProvider = AutoDisposeProvider<double>.internal(
  middleBottomSheetExtent,
  name: r'middleBottomSheetExtentProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$middleBottomSheetExtentHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef MiddleBottomSheetExtentRef = AutoDisposeProviderRef<double>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
